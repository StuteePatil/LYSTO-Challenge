...
# reading image data and their corresponding labels from .h5 file
data = h5py.File('breast.h5', 'r')
X, Y, P = data['images'], np.array(data['counts']), np.array(data['id'])

# color mapping for staining histopathology images
cmap_dab = LinearSegmentedColormap.from_list('mycmap', ['white', 'saddlebrown'])

# converting all images fron rgb to hed color space
img0 = rgb2hed(X[0])
arr = np.array(img0[:,:,2])
img1 = rgb2hed(X[1])
...
# define coefficient of determination (R^2) for regression
...
history = model.fit(X_tr, y_tr, batch_size=100, nb_epoch=100, verbose=1, validation_data=(X_val, y_val), callbacks=[earlystopping])

# evaluating model on test data
y_pred = model.predict(X_tt)

mae_score = mean_absolute_error(y_tt, y_pred)
R2_score = r2_score(y_tt, y_pred)

print("Test Scores")
print('MAE Loss: ', mae_score)
